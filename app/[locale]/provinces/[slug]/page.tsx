...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  const content = getPageBySlug(`/provinces/${slug}`, locale);
  if (content) {
    return { title: content.seo.metaTitle, description: content.seo.metaDescription, alternates: { canonical: canonicalUrl(`/provinces/${slug}`, locale), languages: localeAlternates(`/provinces/${slug}`) } };
  }
  ...
  return {
    ...
    alternates: { canonical: canonicalUrl(`/provinces/${slug}`, locale), languages: localeAlternates(`/provinces/${slug}`) },
  };
}

export default async function ProvinceDetailPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  const { locale, slug } = await params;
  ...
  if (content) {
    ...
    const schema = articleSchema(content.seo?.metaTitle || content.title, content.seo?.metaDescription || content.description, content.lastUpdated, `/provinces/${slug}`, content.lastUpdated);
    return (
      <>
    ...
    <>
      ...
      <div className="space-y-6 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Capital: {province.capital}</p>
        </div>

        <div className="bg-surface border border-border rounded-xl p-5 space-y-4">
          <div>
            <p className="text-xs font-bold text-muted-foreground font-mono uppercase tracking-wider">Regional Office</p>
          ...
          <div>
            <p className="text-xs font-bold text-muted-foreground font-mono uppercase tracking-wider">Payment Collection</p>
            <p className="text-sm text-muted-foreground mt-0.5 leading-relaxed">{province.collectionInfo}</p>
          </div>
        </div>
