...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  const content = getPageBySlug(`/news/${slug}`, locale);
  if (content) {
    return { title: content.seo.metaTitle, description: content.seo.metaDescription, alternates: { canonical: canonicalUrl(`/news/${slug}`, locale), languages: localeAlternates(`/news/${slug}`) } };
  }

  const article = newsArticles.find((a) => a.slug === slug);
  if (!article) return {};
  return { title: `${article.title} | SASSA News`, description: article.summary, alternates: { canonical: canonicalUrl(`/news/${slug}`, locale), languages: localeAlternates(`/news/${slug}`) } };
}
