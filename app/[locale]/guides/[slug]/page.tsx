...
import { breadcrumbSchema, howToSchema, faqSchema, articleSchema } from "../../../../lib/json-ld";
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  const content = getPageBySlug(`/guides/${slug}`, locale);
  if (content) {
    return { title: content.seo.metaTitle, description: content.seo.metaDescription, alternates: { canonical: canonicalUrl(`/guides/${slug}`, locale), languages: localeAlternates(`/guides/${slug}`) } };
  }

  const guide = guides.find((g) => g.slug === slug);
  if (!guide) return {};
  return { title: `${guide.title} | SASSA Step-by-Step Guide`, description: guide.description, alternates: { canonical: canonicalUrl(`/guides/${slug}`, locale), languages: localeAlternates(`/guides/${slug}`) } };
}
...
export default async function GuideDetailPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  if (content) {
    ...
    const schema = articleSchema(content.seo?.metaTitle || content.title, content.seo?.metaDescription || content.description, content.lastUpdated, `/guides/${slug}`, content.lastUpdated);
    return (
      <>
