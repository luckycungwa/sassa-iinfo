...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  const content = getPageBySlug(`/payment-dates/${slug}`, locale);
  if (content) {
    return { title: content.seo.metaTitle, description: content.seo.metaDescription, alternates: { canonical: canonicalUrl(`/payment-dates/${slug}`, locale), languages: localeAlternates(`/payment-dates/${slug}`) } };
  }
  ...
  return {
    ...
    alternates: { canonical: canonicalUrl(`/payment-dates/${slug}`, locale), languages: localeAlternates(`/payment-dates/${slug}`) },
  };
}

export default async function PaymentMonthPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  const { locale, slug } = await params;
  ...
  if (content) {
    ...
    const schema = articleSchema(content.seo?.metaTitle || content.title, content.seo?.metaDescription || content.description, content.lastUpdated, `/payment-dates/${slug}`, content.lastUpdated);
    return (
      <>
