...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  const content = getPageBySlug(`/eligibility/${slug}`, locale);
  if (content) {
    return { title: content.seo.metaTitle, description: content.seo.metaDescription, alternates: { canonical: canonicalUrl(`/eligibility/${slug}`, locale), languages: localeAlternates(`/eligibility/${slug}`) } };
  }

  const guide = eligibilityGuides.find((e) => e.slug === slug);
  if (!guide) return {};
  return { title: `${guide.title} | SASSA Eligibility Guide`, description: guide.shortDescription, alternates: { canonical: canonicalUrl(`/eligibility/${slug}`, locale), languages: localeAlternates(`/eligibility/${slug}`) } };
}

export default async function EligibilityDetailPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  const { locale, slug } = await params;

  const content = getPageBySlug(`/eligibility/${slug}`, locale);
  if (content) {
    const faqBlock = content.contentBlocks.find((b): b is FAQBlock => b.type === "faq");
    const schema = articleSchema(content.seo?.metaTitle || content.title, content.seo?.metaDescription || content.description, content.lastUpdated, `/eligibility/${slug}`, content.lastUpdated);
    return (
      <>
