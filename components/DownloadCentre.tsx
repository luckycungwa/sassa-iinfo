  ...
  return (
    <div className="space-y-6">
        ...
        <div className="lg:col-span-4 space-y-2">
          ...
          {downloadableForms.map((form) => (
            ...
            >
              ...
              <p className={`text-[10px] font-mono ${selectedForm.id === form.id ? "text-amber-300" : "text-accent-dark"}`}>
                {form.approxSize}
              </p>
            </button>
          ))}
        ...
        <div className="lg:col-span-8 bg-surface border border-border rounded-xl p-6 md:p-8  space-y-6">
          ...
          <div className="border-b border-border pb-5 space-y-2 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <button
              ...
              className="bg-accent hover:bg-accent-dark text-white font-bold px-4 py-2 rounded-xl transition text-xs flex items-center gap-1.5"
            >
            ...
            <ul className="space-y-2.5 text-xs md:text-sm text-slate-600">
              {selectedForm.howToFill.map((step, idx) => (
                <li key={idx} className="flex gap-2.5">
                  <span className="w-5 h-5 rounded-full bg-canvas border border-border text-slate-700 text-xs font-mono font-bold flex items-center justify-center flex-shrink-0 mt-0.5">
                    {idx + 1}
                  </span>
      ...
      {isSimulatedPrintOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto print:p-0 print:bg-surface print:static print:z-0">
          <div className="bg-surface rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 md:p-8  space-y-6 relative print:shadow-none print:max-h-full print:p-0">
            ...
            <div className="p-4 bg-canvas rounded-xl border border-border space-y-3 print:hidden">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-muted uppercase font-mono">Full Name & Surname</label>
                  <input
                    type="text"
                    value={simulatedApplicantName}
                    onChange={(e) => setSimulatedApplicantName(e.target.value)}
                    placeholder="e.g. Sipho Nkosi"
                    className="w-full bg-surface border border-slate-200 px-3 py-2 rounded-xl text-xs focus:outline-none focus:ring-1 focus:ring-emerald-800"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-muted uppercase font-mono">13-Digit ID Number</label>
                  <input
                    type="text"
              ...
              <div className="pt-1 flex justify-end">
                <button
                  ...
                  className="bg-accent hover:bg-accent-dark text-white px-4 py-2 rounded-xl transition text-xs font-bold flex items-center gap-1.5"
                >
            ...
            <div className="border border-slate-200 rounded-xl p-8 space-y-6 bg-surface  font-serif text-xs md:text-sm print:border-0 print:p-0 print:shadow-none">
              {/* Header */}
              <div className="text-center border-b-2 border-slate-800 pb-4 space-y-1.5">
                ...
                <p className="text-[10px] font-mono text-muted">REF: {selectedForm.pdfPlaceholderContent}</p>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="border-b border-slate-300 pb-1 flex justify-between">
                    <span className="text-[10px] font-bold uppercase text-muted">Applicant Name:</span>
                    <span className="font-sans font-bold">{simulatedApplicantName || "____________________"}</span>
                  </div>
                  <div className="border-b border-slate-300 pb-1 flex justify-between">
                    <span className="text-[10px] font-bold uppercase text-muted">ID Number:</span>
              ...
              <div className="grid grid-cols-2 gap-8 pt-8 text-center font-sans text-xs">
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-muted">Sign in front of Commissioner</p>
                </div>
                <div className="space-y-1">
                  <div className="border-b border-slate-300 h-10"></div>
                  <p className="font-semibold text-muted">SASSA Commissioner / Officer</p>
                  <p className="text-[10px] text-muted">Date and Stamp</p>
                </div>
              </div>
            </div>
          </div>
