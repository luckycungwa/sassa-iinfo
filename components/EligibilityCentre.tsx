  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-4 space-y-4">
        <div className="bg-emerald-900 text-white rounded-2xl p-5 border border-emerald-800 shadow-sm space-y-3">
          ...
          <button
            ...
            className="w-full bg-amber-400 hover:bg-amber-500 text-emerald-950 font-bold px-4 py-2 rounded-xl transition text-xs shadow"
          >
        ...
        <div className="space-y-3">
          ...
          <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {selectedGuide.checklist.map((item, idx) => (
              <li key={idx} className="p-3 bg-slate-50 border border-slate-100 rounded-xl flex items-start gap-2.5 text-xs md:text-sm text-slate-700">
            ...
            {selectedGuide.recommendedGrants.map((grant, idx) => (
              <div key={idx} className="p-4 border border-emerald-100 bg-emerald-50/30 rounded-xl flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">Estimated payout amount</p>
                </div>
                <div className="font-extrabold font-mono text-sm text-emerald-800 bg-white border border-emerald-100 px-3 py-1 rounded-lg">
                  {grant.amount}
                </div>
              </div>
            ))}
